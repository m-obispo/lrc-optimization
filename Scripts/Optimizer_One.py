...
for k in range(1):
    ...
    N = np.array(N)
    R = np.array(R)
    MP4 = np.vstack((MP4,np.array(mp4SDTQ)))
...
def SEP(omega):
    global R, DFT, MP4, dE
    SCF = np.zeros(21)
    geraEntrada(omega)       #Gera as entradas a serem utilizadas pelo Gaussian 09
    t0 = time.time()
    os.system('bash roda-um.sh')  #Executa os cálculos do Gaussian 09, um por vez.
...
ti = time.time()
resultado = opt.minimize(SEP, 0.3, method="Nelder-Mead", tol = 1e-7, options = {'eps':1e-3})
tf = time.time() - ti

wOpt = resultado['x']# = 0.25111888 para wb97xd[0]
...
with open('../resultado_'+funct+'_CORRIGIDO.txt', 'w') as h:
